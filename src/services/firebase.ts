...
async getSupplierCustomers(supplierId: string): Promise<RestaurantCustomer[]> {
  ...
  listenToCustomerNotifications(userId: string, tableNumber: string, callback: (notifications: any[]) => void): () => void {
    const q = query(
  ...
  async markNotificationAsRead(notificationId: string): Promise<void> {
    try {
      const docRef = doc(db, 'customerNotifications', notificationId);
      await updateDoc(docRef, {
        read: true,
